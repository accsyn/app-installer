...
public class ASIWriteCredentialsAction extends AbstractInstallOrUninstallAction {
	...
	private boolean execute(Context context) {
		...
		} else if (client_id.equalsIgnoreCase("null")) {
			ASICommon.warning("Not writing server credentials to temp file - auth skipped by user!");
			return true;
		}
		...
		try {
			...
			ASICommon.info("Writing client credentials to disk @ '" + f_credentials.getName() + "'");
			f_credentials.getParentFile().mkdirs();
			writer = new PrintWriter(f_credentials.getAbsolutePath(), "UTF-8");
			writer.println(client_id);
			writer.close();
			ASICommon.info("Successfully wrote client credentials.");
